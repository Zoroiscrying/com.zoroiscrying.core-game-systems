...
{
    /// <summary>
    /// Responsible for notifying all Savable components
    /// Asking them to send data or retrieve data from/to the GameSaveData
    /// </summary>
    [AddComponentMenu(""), DefaultExecutionOrder(-9015)]
    public class SaveSystemPersistentManager : MonoBehaviour
    {
        
        private static SaveSystemPersistentManager instance;

        private static GameObject saveManagerTemplate;
        ...
        private static GameSaveData activeGameSaveData = null;
        private static int activeSlot = -1;
        
        // All listeners
        private static List<SavableBehavior> savables = new List<SavableBehavior>();
        
        [RuntimeInitializeOnLoadMethod(RuntimeInitializeLoadType.BeforeSceneLoad)]
        private static void CreateInstance()
        {
            GameObject saveMasterObject = new GameObject("Save Master");
            saveMasterObject.AddComponent<SaveSystemPersistentManager>();
        ...
        */
        ...
        /// <summary>
        /// If one scene is unloaded,
        /// </summary>
        /// <param name="scene"></param>
        private static void OnSceneUnloaded(Scene scene)
        {
            if (activeGameSaveData == null)
                return;
        ...
        /// <summary>
        /// If one scene is loaded, check for duplicated scenes
        /// </summary>
        /// <param name="scene"></param>
        ...
        private static void OnSceneLoaded(Scene scene, LoadSceneMode arg1)
        {
            ...
            if (string.IsNullOrEmpty(activeGameSaveData.Get(string.Format("SaveMaster-{0}-IM", scene.name))))
            {
                return;
            }
        ...
        public static SaveInstanceManager SpawnInstanceManager(Scene scene, string id = "")
        {
            ...
            var savable = go.AddComponent<SavableBehavior>();
            SceneManager.MoveGameObjectToScene(go, scene);

            string saveID = string.IsNullOrEmpty(id) ? scene.name : string.Format("{0}-{1}", scene.name, id);

            savable.SaveIdentification = string.Format("{0}-{1}", "SaveMaster", saveID);
            savable.AddSavableComponent("IM", instanceManager, true);
            saveInstanceManagers.Add(scene.GetHashCode(), instanceManager);

            instanceManager.SceneID = saveID;
            instanceManager.Savable = savable;
        ...
        /// Will return false if it has been destroyed due to game-exiting or scene-unloading.
        /// </summary>
        /// <param name="gameObject"></param>
        /// <returns></returns>
        public static bool DeactivatedObjectExplicitly(GameObject gameObject)
        {
            return gameObject.scene.isLoaded && !SaveSystemPersistentManager.isQuittingGame;
        }
        
        /// <summary>
        /// Returns the active slot. -1 means no slot is loaded
        /// </summary>
        /// <returns> Active slot index </returns>
        ...
        /// <summary>
        /// Check if a slot index has been used
        /// </summary>
        ...
        public static void SetSlotAndCopyActiveSave(int slot)
        {
            ...
            activeSlot = slot;
            activeGameSaveData = SaveFileUtility.LoadSave(slot, true);
        ...
        public static void SetSlot(int slot, bool reloadSaveables, GameSaveData saveGame = null)
        {
            ...
            if (SaveSettings.Get().autoSaveOnSlotSwitch && activeGameSaveData != null)
            {
            ...
            activeSlot = slot;
            activeGameSaveData = (saveGame == null) ? SaveFileUtility.LoadSave(slot, true) : saveGame;
        ...
        public static DateTime GetSaveCreationTime(int slot)
        {
            if (slot == activeSlot)
            {
                return activeGameSaveData.creationDate;
            }
        ...
        public static TimeSpan GetSaveTimePlayed(int slot)
        {
            if (slot == activeSlot)
            {
                return activeGameSaveData.timePlayed;
            }
        ...
        public static int GetSaveVersion(int slot)
        {
            if (slot == activeSlot)
            {
                return activeGameSaveData.gameVersion;
            }
        ...
        private static GameSaveData GetSave(int slot, bool createIfEmpty = true)
        {
            if (slot == activeSlot)
            {
                return activeGameSaveData;
            }
        ...
        public static void WriteActiveSaveToDisk()
        {
            ...
            if (activeGameSaveData != null)
            {
                ...
                {
                    savables[i].OnSaveRequest(activeGameSaveData);
                }

                SaveFileUtility.WriteSave(activeGameSaveData, activeSlot);
            }
            else
            {
        ...
        public static void WipeSceneData(string name, bool clearSceneSaveables = true)
        {
            ...
            {
                int listenerCount = savables.Count;
                for (int i = listenerCount - 1; i >= 0; i--)
                {
                    if (savables[i].gameObject.scene.name == name)
                    {
                        savables[i].WipeData(activeGameSaveData);
                    }
                }
            }

            activeGameSaveData.WipeSceneData(name);
        }
        ...
        public static void WipeSaveable(SavableBehavior saveable)
        {
            if (activeGameSaveData == null)
            {
                Debug.LogError("Failed to wipe scene data: No save game loaded.");
            ...
            saveable.WipeData(activeGameSaveData);
        }
        ...
        {
            if (notifySave && activeGameSaveData != null)
            {
                int saveableCount = savables.Count;
        ...
        public static void ReloadListener(SavableBehavior savable)
        {
            savable.OnLoadRequest(activeGameSaveData);
        }
        
        /// <summary>
        /// Add saveable from the notification list. So it can recieve load/save requests.
        /// </summary>
        /// <param name="savable"> Reference to the saveable that listens to the Save Master </param>
        public static void AddListener(SavableBehavior savable)
        {
            if (savable != null && activeGameSaveData != null)
            {
                savable.OnLoadRequest(activeGameSaveData);
            }
        ...
        /// <summary>
        /// Add savable from the notification list. So it can receive load/save requests.
        /// </summary>
        /// <param name="savable"> Reference to the savable that listens to the Save Master </param>
        public static void AddListener(SavableBehavior savable, bool loadData)
        {
            if (loadData)
            {
                AddListener(savable);
        ...
        /// <summary>
        /// Remove savable from the notification list. So it no longers recieves load/save requests.
        /// </summary>
        /// <param name="savable"> Reference to the saveable that listens to the Save Master </param>
        public static void RemoveListener(SavableBehavior savable)
        {
            if (savables.Remove(savable))
            {
                if (savable != null && activeGameSaveData != null)
                {
                    savable.OnSaveRequest(activeGameSaveData);
                }
            }
        }
        ...
        public static void RemoveListener(SavableBehavior savable, bool saveData)
        {
            if (saveData)
            {
                RemoveListener(savable);
            }
            else
            {
        ...
        public static void SyncLoad()
        {
            ...
            for (int i = 0; i < count; i++)
            {
                savables[i].OnLoadRequest(activeGameSaveData);
            }
        }
        ...
        public static bool GetSaveableData<T>(int slot, string saveableId, string componentId, out T data)
        {
            ...
            GameSaveData saveGame = SaveSystemPersistentManager.GetSave(slot, false);
        ...
        public static void SetInt(string key, int value)
        {
            ...
            activeGameSaveData.Set(string.Format("IVar-{0}", key), value.ToString(), "Global");
        }
        ...
        public static int GetInt(string key, int defaultValue = -1)
        {
            ...
            var getData = activeGameSaveData.Get(string.Format("IVar-{0}", key));
        ...
        public static void SetFloat(string key, float value)
        {
            ...
            activeGameSaveData.Set(string.Format("FVar-{0}", key), value.ToString(), "Global");
        }
        ...
        public static float GetFloat(string key, float defaultValue = -1)
        {
            ...
            var getData = activeGameSaveData.Get(string.Format("FVar-{0}", key));
        ...
        public static void SetString(string key, string value)
        {
            ...
            activeGameSaveData.Set(string.Format("SVar-{0}", key), value, "Global");
        }
        ...
        private IEnumerator TrackHotkeyUsage()
        {
            ...
            while (true)
            {
                ...
                {
                    SaveSystemPersistentManager.WipeSceneData(SceneManager.GetActiveScene().name);
                }
        ...
        private IEnumerator IncrementTimePlayed()
        {
            ...
            while (true)
            {
                ...
                if (activeSlot >= 0)
                {
                    activeGameSaveData.timePlayed = activeGameSaveData.timePlayed.Add(TimeSpan.FromSeconds(1));
                }
            }
        }
